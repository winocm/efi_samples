	...
	for (;;) {
		...
		if (tok->prompt != NULL) {
			...
			if (new == NULL)
				...
				int start = (int)(tok->start - tok->buf);
				int oldlen = (int)(tok->cur - tok->buf);
#else
				int start = tok->start - tok->buf;
				int oldlen = tok->cur - tok->buf;
#endif
				int newlen = oldlen + (int)strlen(new);
			...
			while (!done) {
#if defined(EFI32) || defined(EFI64)
				int curstart = (int)(tok->start == NULL ? -1 :
					       tok->start - tok->buf);
				int curvalid = (int)(tok->inp - tok->buf);
#else
	...
	char **p_start, **p_end; /* Out: point to start/end of token */
{
 ...
 again:
  ...
  letter_quote:
	...
	if (c == '\'' || c == '"') {
#if defined(EFI32) || defined(EFI64)
		int quote2 = (int) (tok->cur - tok->start + 1);
#else
