	...
	void *p;
{
	...
	return PyInt_FromLong((long)((UINTN)p));
#else
	...
	PyObject *vv;
{
	...
	while (--i >= 0) {
		prev = x;
#if defined(EFI32) || defined(EFI64)
		x = LShiftU64(x, SHIFT) + v->ob_digit[i];
		if ((LONG_LONG)RShiftU64(x, SHIFT) != prev) {
#else
		x = (x << SHIFT) + v->ob_digit[i];
		if ((x >> SHIFT) != prev) {
#endif
	...
	for (j = size_v, k = a->ob_size-1; a != NULL && k >= 0; --j, --k) {
		...
		for (i = 0; i < size_w && i+k < size_v; ++i) {
			...
			carry += v->ob_digit[i+k] - z
				+ ((twodigits)zz << SHIFT);
#if defined(EFI32) || defined(EFI64)
            v->ob_digit[i+k] = (unsigned short)(carry & MASK);
#else
