...
#else
    ret = fseek(f->f_fp, (long)offset, whence);
#endif
	Py_END_ALLOW_THREADS
	if (ret != 0) {
...
#if defined(EFI32) || defined(EFI64)
		pos = (long)lseek(fileno(f->f_fp), 0L, SEEK_CUR);
#else
	...
	PyObject *args;
{
	...
	v = PyString_FromStringAndSize((char *)NULL, (int)buffersize);
	if (v == NULL)
		return NULL;
	bytesread = 0;
	for (;;) {
		...
		if (bytesrequested < 0) {
			...
			if (_PyString_Resize(&v, (int)buffersize) < 0)
				return NULL;
		}
	}
	if (bytesread != buffersize)
		_PyString_Resize(&v, (int)bytesread);
	return v;
}
	...
	PyObject *args;
{
	...
	while (ntodo > 0) {
		Py_BEGIN_ALLOW_THREADS
		errno = 0;
		nnow = (int)fread(ptr+ndone, 1, ntodo, f->f_fp);
		Py_END_ALLOW_THREADS
		if (nnow == 0) {
			if (!ferror(f->f_fp))
				break;
	...
	PyObject *args;
{
	...
	for (;;) {
		...
		if (p == NULL) {
			...
			if (big_buffer == NULL) {
				...
				/* Grow the big buffer */
				_PyString_Resize(&big_buffer, (int)buffersize);
				buffer = PyString_AS_STRING(big_buffer);
			}
			continue;
		}
		...
		do {
			...
			p++;
#if defined(EFI64)
			line = PyString_FromStringAndSize(q, (int)(p-q));
#else
	...
	if (nfilled != 0) {
		/* Partial last line */
		line = PyString_FromStringAndSize(buffer, (int)nfilled);
		if (line == NULL)
			goto error;
		if (sizehint > 0) {
