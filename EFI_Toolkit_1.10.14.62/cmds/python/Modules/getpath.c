...
calculate_path()
{
...
#endif
#if defined(EFI32) || defined(EFI64)
    sprintf(lib_python, "python%s\\lib", VERSION);
#else
	...
	if (rtpypath)
		bufsz += (int)strlen(rtpypath) + 1;

	prefixsz = (int)strlen(prefix) + 1;
	...
	while (1) {
		...
		if (delim)
#if defined(EFI32) || defined(EFI64)
			bufsz += (int)(delim - defpath) + 1;
#else
			bufsz += delim - defpath + 1;
#endif
		else {
			bufsz += (int)strlen(defpath) + 1;
			break;
		}
		defpath = delim + 1;
	}

    if (strlen(exec_prefix) > 0)
	    bufsz += (int)strlen(exec_prefix) + 1;
	...
	if (buf == NULL) {
		...
		while (1) {
			...
			if (delim) {
#if defined(EFI32) || defined(EFI64)
                int len = (int)(delim - defpath) + 1;
#else
				int len = delim - defpath + 1;
#endif
				int end = (int)strlen(buf) + len;
