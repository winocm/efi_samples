...
size = (int)(length + offsetof(real_pcre, code[0]));
...
static int grow_stack(match_data *md)
{
  if (md->length != 0)
    ...
    {
      int string_len = (int)(md->end_subject - md->start_subject + 1);
...
for (;;)
  ...
  if ((int)*ecode >= OP_BRA)
    ...
    if (number > 0 && number < md->offset_end)
      ...
      md->offset_vector[number] = (int)(eptr - md->start_subject);
    ...
    case OP_KETRMAX:
      ...
      if (number > 0)
        ...
        if (number >= md->offset_end) md->offset_overflow = TRUE; else
          {
          md->offset_vector[number+1] = (int)(eptr - md->start_subject);
