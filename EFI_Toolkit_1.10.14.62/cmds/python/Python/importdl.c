...
/* Explanation of some of the the various #defines used by dynamic linking...
   ...
   USE_EFI  -- Dynamic loading via EFI protocols
...
#define USE_EFI
#define DYNAMIC_LINK
#define SHORT_EXT ".pym"
#define LONG_EXT "module.pym"

#include "PythonModuleProtocol.h"
#include <efilib.h>
#include <unistd.h>

extern EFI_PYTHON_API EfiPythonApi;
static EFI_PYTHON_MODULE_INTERFACE *ModuleInterface;
static PyObject *efi_dyn_images = NULL;

static EFI_STATUS efi_unload_image_fun(  IN EFI_HANDLE ImageHandle );
static void efi_init_dyn( void );
static void efi_cleanup_dyn( void );
static void efi_init_wrapper( void);
static void efi_nuke_dyn( PyObject *item );
static void efi_add_dyn( char *pathname, EFI_HANDLE id );
static void efi_init_wrapper( void);
...
*/
...
#ifdef USE_EFI
    {
        ...
        EFI_GUID PythonGuid = EFI_PYTHON_MODULE_PROTOCOL;
        wchar_t wPathname[FILENAME_MAX];
        
        /* Load and start this module. If the module loads successfully it
         *  will install the module interface protocol on its image handle.
         */
        if (strlen(pathname) < FILENAME_MAX) {
            mbstowcs(wPathname, pathname, FILENAME_MAX);
            Status = LoadImage(wPathname, &ImageHandle);
            if (!EFI_ERROR(Status)) {
                UINT32 LoadOptionsSize = 0;
                wchar_t *wCwd, *LoadOptions = NULL;
                wCwd = wgetcwd(0, 0);
                if (wCwd) {
                    LoadOptionsSize = (UINT32)(wcslen(wPathname) + 1 + wcslen(wCwd) + 1) * sizeof(wchar_t);
                    LoadOptions = calloc(LoadOptionsSize, 1);
                    if (!LoadOptions) {
                        LoadOptionsSize = 0;
                    } else {
                        wcscpy(LoadOptions, wPathname);
                        wcscat(&LoadOptions[wcslen(LoadOptions) + 1], wCwd);
                    }
                    free(wCwd);
                }
                Status = StartImage(ImageHandle, LoadOptionsSize, LoadOptions, _LIBC_EFISystemTable, NULL, NULL);
                free(LoadOptions);
                if (Py_VerboseFlag && EFI_ERROR(Status) ) {
                    printf( "Start image ( %s ) failed", pathname);
                }
            } else {
			    if( Py_VerboseFlag ) {
				    printf( "Load image ( %s ) failed", pathname );
			    }
            }
        } else {
            Status = EFI_INVALID_PARAMETER;
			if( Py_VerboseFlag ) {
				printf( "Filename too long ( %s )", pathname );
			}
        }

        /* Now get the protocol interface from the image handle.
         */
        ModuleInterface = NULL;
        if (!EFI_ERROR(Status)) {
            Status = _LIBC_EFISystemTable->BootServices->HandleProtocol(ImageHandle, &PythonGuid, (void**)&ModuleInterface);
			if( /*Py_VerboseFlag && */EFI_ERROR(Status) ) {
				printf( "Handle protocol ( %s ) failed", pathname );
			}
        }
#else
        {
            ...
            EFI_GUID PythonModuleGuid = EFI_PYTHON_MODULE_PROTOCOL;
        
            DEBUG ((D_LOAD, "Calling LibLocatehandle\n"));
            Status = LibLocateHandle(ByProtocol, &PythonModuleGuid, NULL, &NumHandles, &Buffer);
            DEBUG ((D_LOAD, "Located %d handles\n", NumHandles));
            if (!EFI_ERROR(Status)) {
                char    *Name;
                UINTN     i;
                for (i = 0; i < NumHandles; ++i) {
                     ...
                     */
                    DEBUG ((D_LOAD, "Calling HandleProtocol (%d)\n", i));
                    Status = _LIBC_EFISystemTable->BootServices->HandleProtocol(Buffer[i], &PythonModuleGuid, &ModuleInterface);
                    if (!EFI_ERROR(Status)) {
                        DEBUG ((D_LOAD, "Getting module name\n"));
                        Status = ModuleInterface->ModuleName(ModuleInterface, &Name);
                    }
                    else DEBUG ((D_LOAD, "Error on HandleProtocol call %r\n", Status));
                    if (!EFI_ERROR(Status)) {
                        DEBUG ((D_LOAD, "Name is '%s'\n", Name));
                        if (strcmp(name, Name) == 0) {
                            ImageHandle = Buffer[i];
                            break;
                        }
                    }
                    else DEBUG ((D_LOAD, "Error on ModuleName call %r\n", Status));
                    ModuleInterface = NULL;
                }
            }
        }
#endif
        /* Check the revision level on the module's interface.
         */
        if (!EFI_ERROR(Status) && ModuleInterface) {
            if (ModuleInterface->Revision > EFI_PYTHON_MODULE_INTERFACE_REVISION) {
			    char buff[256]; 

                ModuleInterface = NULL;
    		    sprintf( buff, "EFI: Protocol revision mismatch on %.200s", pathname );
			    PyErr_SetString( PyExc_ImportError, buff );
			    return NULL;
            }
        }

        /* If everything succeeded get set up to call the module's init function
         *  and record the module's name and handle so it can be unloaded.
         */
        if (!EFI_ERROR(Status) && ModuleInterface) {
            p = efi_init_wrapper;
            efi_add_dyn(name, ImageHandle);
        } else {
			char buff[256]; 

    		sprintf( buff, "EFI: Failed to load %.200s %p %p", pathname, (void*)Status, ModuleInterface );
    		if (ImageHandle != NULL)  { 
    		//
    		//  Unload the image.  
    		//  UnloadImage boot service require image has unload routine
    		//  If image unload routine is not available, set a dummy unload routine 
    		//
			EFI_LOADED_IMAGE  *ImageInfo=NULL;
			EFI_STATUS	Status0;
			Status0= _LIBC_EFISystemTable->BootServices->HandleProtocol(
									ImageHandle,
									&LoadedImageProtocol,
									&ImageInfo
			 						);

                        if (!EFI_ERROR(Status0) && ImageInfo != NULL && ImageInfo->Unload == NULL){
                                ImageInfo->Unload = efi_unload_image_fun;
                        }
